...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.start = unetConv2(
            ...
            filters[0]
            if not concat_x
            else filters[0] - num_input_channels,
            norm_layer,
            need_bias,
            pad,
        )
        ...
        self.down1 = unetDown(
            ...
            filters[1]
            if not concat_x
            else filters[1] - num_input_channels,
            norm_layer,
            need_bias,
            pad,
        )
        self.down2 = unetDown(
            ...
            filters[2]
            if not concat_x
            else filters[2] - num_input_channels,
            norm_layer,
            need_bias,
            pad,
        )
        self.down3 = unetDown(
            ...
            filters[3]
            if not concat_x
            else filters[3] - num_input_channels,
            norm_layer,
            need_bias,
            pad,
        )
...
class unetConv2(nn.Module):
    def __init__(
        self, in_size, out_size, norm_layer, need_bias, pad
    ):
        ...
        if norm_layer is not None:
            ...
            self.conv2 = nn.Sequential(
                conv(
                    out_size, out_size, 3, bias=need_bias, pad=pad
                ),
                norm_layer(out_size),
                nn.ReLU(),
            )
...
class unetDown(nn.Module):
    def __init__(
        ...
        self.conv = unetConv2(
            in_size, out_size, norm_layer, need_bias, pad
        )
...
class unetUp(nn.Module):
    def __init__(
        ...
        if upsample_mode == "deconv":
            ...
            self.conv = unetConv2(
                out_size * 2, out_size, None, need_bias, pad
            )
        elif (
            ...
            self.up = nn.Sequential(
                ...
                conv(
                    num_filt, out_size, 3, bias=need_bias, pad=pad
                ),
            )
            self.conv = unetConv2(
                out_size * 2, out_size, None, need_bias, pad
    ...
    def forward(self, inputs1, inputs2):
        ...
        if (inputs2.size(2) != in1_up.size(2)) or (
            inputs2.size(3) != in1_up.size(3)
        ):
            ...
            inputs2_ = inputs2[
                :,
                :,
                diff2 : diff2 + in1_up.size(2),
                diff3 : diff3 + in1_up.size(3),
            ]
        else:
            inputs2_ = inputs2
